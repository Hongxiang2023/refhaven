...
5. If an older unpacked connector was loaded from the application bundle and disappeared, open **Library & connector → Copy folder path** and load the new stable connector folder once. Pair it with **Copy connector pairing key**, then remove the old browser entry if it still appears. Future app replacements keep this folder path. An already-loaded Folio connector uses the same protocol and remains compatible. The Chrome Web Store version is pending review; when it becomes available, it will need to be paired once as a separate browser extension before you remove the unpacked copy.
