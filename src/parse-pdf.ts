...
export async function parsePdf(pdfId:string,signal:AbortSignal,onProgress:(message:string)=>void,previous?:ReadingCache,context:ReadingContext={}):Promise<ReadingCache>{
 ...
 try{
  ...
  const result:ReadingCache={version:1,layoutVersion:20,pages:[],figures:[],warnings:['Text order and figure captions are estimated from the PDF. Figure boundaries are estimated; the original PDF remains available.']};
